...
function downloadQR(id: string, nombre: string) {
  ...
  a.href = url; a.download = `QR_${id}_${nombre.replace(/\s+/g, '_')}.svg`; a.click()
  URL.revokeObjectURL(url)
}

function printQRs(trabajadores: Trabajador[]) {
  const win = window.open('', '_blank')
  if (!win) return
  const cards = trabajadores.map(t => {
    const svg = document.getElementById(`qr-${t.id}`)
    const svgData = svg ? new XMLSerializer().serializeToString(svg) : ''
    const nombre1 = t.nombre.split(' ').slice(0, 2).join(' ')
    const nombre2 = t.nombre.split(' ').slice(2).join(' ')
    return `<div class="card">${svgData}<div class="nom">${nombre1}</div>${nombre2 ? `<div class="nom">${nombre2}</div>` : ''}<div class="carg">${t.cargo}</div><div class="id">ID: ${t.id}</div></div>`
  }).join('')
  win.document.write(`<!DOCTYPE html><html><head><style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .card svg{width:130px!important;height:130px!important}
    .nom{font-size:9px;font-weight:700;margin-top:6px;line-height:1.3;color:#111}
    .carg{font-size:8px;color:#666;margin-top:2px}
    .id{font-size:9px;color:#d97706;font-family:monospace;margin-top:3px;font-weight:700}
    @media print{@page{margin:8mm}.grid{gap:6px;padding:8px}}
  </style></head><body>
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-k-text2 text-sm">
          Cada QR encodea el <span className="text-k-amber font-bold">ID numérico</span> del trabajador. Si cambia el nombre o cargo, el QR físico sigue siendo válido.
        </p>
        <button
          onClick={() => printQRs(filtered)}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filtered.map(t => (
            <div key={t.id}
              ...
              {/* QR Code */}
              <div className="bg-white rounded-lg p-2 mb-3 w-[130px] h-[130px] flex items-center justify-center flex-shrink-0">
                <QRCode
                  id={`qr-${t.id}`}
                  value={t.id}
                  size={114}
              ...
              {/* Descargar */}
              <button
                onClick={() => downloadQR(t.id, t.nombre)}
